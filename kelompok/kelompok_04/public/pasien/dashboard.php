...
// Proses ambil antrian (REVISI: berdasarkan poli, jam, tanggal)
...
<body class="min-h-screen bg-gray-50">
            ...
            <div class="bg-gradient-to-r from-[#45BC7D] to-[#3aa668] rounded-2xl p-6 text-white shadow-md">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex items-center gap-2">
                        ...
                        <span>
                            Estimasi: <?php echo $estimasiMenit !== null ? $estimasiMenit . ' menit' : '-'; ?>
                        </span>
                    </div>
        ...
        <div class="bg-white rounded-2xl w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form method="post" action="" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" id="inputTanggal" min="<?php echo date('Y-m-d'); ?>" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#45BC7D] focus:border-transparent" required />
                </div>
                ...
                <div id="jamContainer" class="hidden">
                    <label class="block text-sm text-gray-700 mb-3">Pilih Jam Praktik</label>
                    <div id="jamList" class="space-y-2"></div>
                    <p id="jamKosong" class="text-sm text-gray-500 hidden">Tidak ada jadwal tersedia untuk poli dan tanggal yang dipilih.</p>
                </div>
    ...
    <script>
        ...
        function updateJamList() {
            ...
            const hariIdx = selectedDate.getDay();
            ...
            jadwalFiltered.forEach(jadwal => {
                const jamMulai = jadwal.jam_mulai.substring(0, 5);
                const jamSelesai = jadwal.jam_selesai.substring(0, 5);
                
                const btn = document.createElement('button');
                btn.type = 'button';
                btn.className = 'w-full px-4 py-3 bg-white border-2 border-gray-200 rounded-xl hover:border-[#45BC7D] hover:bg-[#45BC7D]/5 transition-all text-left flex items-center justify-between';
                btn.innerHTML = `
                    <div>
                        <p class="text-sm font-medium text-gray-800">${jamMulai} - ${jamSelesai}</p>
                        <p class="text-xs text-gray-500 mt-1">Klik untuk pilih jam ini</p>
                    </div>
        ...
        selectPoli.addEventListener('change', updateJamList);
        inputTanggal.addEventListener('change', updateJamList);
    </script>
</body>
</html>
